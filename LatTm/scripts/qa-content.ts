...
async function main() {
  ...
  if (manifest.chapters.length !== 20) {
    issues.push({
      level: "error",
      file: relative(manifestPath),
      message: `장/부록 개수가 20개가 아닙니다. 현재 ${manifest.chapters.length}개입니다.`
    });
  }
  ...
  issues.push(...(await detectDuplicateParagraphs(manifest)));
...
async function detectDuplicateParagraphs(manifest: Manifest) {
  ...
  for (const entry of manifest.chapters) {
    ...
    for (const paragraph of paragraphs) {
      ...
      const files = paragraphMap.get(normalized) ?? [];
      files.push(relative(filePath));
      paragraphMap.set(normalized, files);
    }
  }
  ...
  for (const [paragraph, files] of paragraphMap.entries()) {
    ...
    if (uniqueFiles.length > 1) {
      issues.push({
        ...
        message: `길이가 긴 중복 문단이 감지되었습니다: "${paragraph.slice(0, 80)}..."`
      });
    }
  }
