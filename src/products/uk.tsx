...
const {
  ...
  topbarKicker: "UKTm Beta",
  loadingMessage: "영국 상표 가이드 콘텐츠를 불러오는 중입니다.",
  homeHeroKicker: "영국 단일국가 early-track verified guide",
  homeSummary: (
    <>
      영국 시장 상표 실무를 빠르게 점검하기 위한 early-track verified 공개본입니다. EU 공통 프레임과 병행해
      읽고 싶다면 <Link to={europePath}>EuTm</Link>으로 큰 구조를 먼저 잡은 뒤, 영국 단일국가
      판단이 필요한 순간 이 트랙으로 내려오는 구성이 자연스럽습니다. 현재는 대형 확장보다
      `early-track filing decision board`, `beta-lane maintenance owner board`, `online incident quick board`를
      빠르게 다시 보는 reader utility와 fact-verification / standard-QA 정합 유지에 초점을 둡니다.
    </>
  ),
  positioningKicker: "UKTm Positioning",
  positioningTitle: "영국 단일 시장 실무를 early-track verified 공개본으로 유지하는 beta guide",
  positioningNote: (
    <>
      현재 UKTm은 UKIPO 중심 출원 흐름, 사용증거와 non-use cancellation, 영국 온라인 침해와
      분쟁 대응을 단일 시장 기준으로 빠르게 점검하는 용도에 맞춰져 있습니다. 지금 라운드에서는 대형
      확장보다 `early-track filing decision board`, `beta-lane maintenance owner board`, `online incident quick board`를
      반복 검증하며 beta / standard QA 기준의 verified 공개본 안정성과 다음 refresh 입력 정합화에 우선을 둡니다.
    </>
  ),
  chapterBadge: "UK",
