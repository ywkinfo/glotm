...
export const briefCandidates: BriefCandidate[] = [
  ...
  },
  {
    ...
    trigger:
      "특허청(JPO) 「令和8年熊本地震により影響を受けた手続の取扱いについて」(2026-07-29 게시, 2026-08-07 갱신). 지정기간은 상표심사에서 기간 도과 후 제출을 인정하고, 법정기간은 ⓐ 원칙 14일(在外者는 2개월) 구제군과 ⓑ 「故意によるものでないこと」 기준 2개월 구제군으로 갈린다. ⓑ에 상표권 존속기간 갱신등록 신청(商21조1항)이 들어 있다. **[2026-09-13 정정]** 이 자리에 처음 적은 `소정기간 경과 후 1년 이내`는 틀렸다 — 1년은 ⓑ의 일반 상한이고, 그 바로 뒤 괄호 `（（キ）から（ケ）までについては所定期間経過後6月以内）`가 (キ) 商21조1항에 **6개월**을 붙인다. 상세는 notes ⓒ.",
    discoveredOn: "2026-09-12",
    sourceIds: ["jpo"],
    jurisdictions: ["Japan", "Deadline Relief"],
    relatedProductSlugs: ["japan"],
    status: "published",
    publishedAs: "2026-09-jpo-kumamoto-deadline-relief",
    notes:
      "2026-09-12 sweep에서 JPO 災害関連情報 면과 본문을 직접 열어 확인했다. **이 소재는 직전 회차가 놓친 자리에서 나왔다** — 2026-08-30 sweep은 jpo의 sweepTarget을 報道発表 2026年度 목록으로만 잡아 「상표 실무 트리거 없음」으로 닫았는데, 이 고지는 報道発表가 아니라 広報／災害関連情報에 실린다. sweepTarget이 목록 하나를 가리키면 그 목록 밖은 정의상 안 보인다. JapTm은 커버리지 공백이 95일로 포트폴리오 최장이고 열린 후보가 0건이었다. **발행 전에 확인할 것**: ⓐ 이 구제가 熊本지진 피해자에 한정되는지 아니면 조문상 일반 재해 구제 틀(災害等における手続救済の基本的考え)의 발동 사례인지 — 후자라면 본문 가치가 이번 지진을 넘어선다, ⓑ 한국 출원인이 실제로 '影響を受けた'에 해당하는 범위(일본 대리인 업무 중단 등 간접 영향 포함 여부), ⓒ 特定非常災害特別措置法 제3조3항 기간연장은 별도 고지이므로 두 경로를 섞지 않는다. 법률 사실 판단과 발행 여부는 owner 몫이다. **2026-09-13 WebSearch triage(1차 출처 미열람)**: ⓐ에 관해 「正当な理由」·「故意によるものでないこと」 두 트랙과 四法共通 가이드라인이 상설로 존재하고 `責めに帰することができない理由`의 14일(在外者 2개월)도 그 상설 틀 안에 있다는 검색 결과가 나왔다 — 이번 지진 고지가 상설 틀의 발동 사례라는 쪽을 지지하지만 본문을 열지 않았으므로 확정이 아니다. 함께 확인할 것 둘이 더 생겼다: ⓓ 검색이 돌려준 `/news/koho/saigai/kumamotojishin2026_sochi/`는 제목상 **각국·지역 지재청의** 구제조치(일본 출원인의 해외 출원 방향)라 이 후보가 근거로 삼는 국내 고지와 다른 문서다 — 최소 세 건이 병존하므로 본문에서 섞지 않는다, ⓔ 검색이 돌려준 `saigai-encho-20260807.html`이 ⓒ의 特措法 고지 자체라 trigger의 `2026-08-07 갱신`이 국내 고지의 갱신일이 아니라 이 별도 고지의 게시일일 수 있다. **2026-09-13 1차 출처 대조로 발행 전 확인 항목을 닫았다(캡처 근거 — `docs/briefs-discovery.md` `캡처 근거 대조`).** ⓐ **상설 틀의 발동이 맞다.** 국내 고지가 법정기간 구제를 「その責めに帰することができない理由」(＜1＞)와 「故意によるものでないこと」(＜2＞) 두 트랙으로 나누고 각 트랙을 `法令上…定められているもの`으로 명시한다 — 지진이 만든 제도가 아니라 조문에 있는 구제를 이번 재해에 적용하는 안내다. 따라서 본문 가치가 이번 지진을 넘어선다. ⓑ ＜1＞은 `手続が可能となってから14日以内（在外者の場合は2月以内（（キ）について在外者の場合は1月以内））`이고 바깥 경계가 `所定期間経過後6月以内`다. **ⓒ 이 후보 노트의 `1년`은 틀렸다 — 상표 갱신은 6개월이다.** ＜2＞의 일반 상한이 `所定期間経過後1年以内`인 것은 맞지만 곧바로 `（（キ）から（ケ）までについては所定期間経過後6月以内）`가 붙고, (キ)가 바로 `商標権の存続期間の更新登録の申請（商21条1項）`다. 상표 실무에서 쓰는 숫자는 **2개월 이내 신청 · 소정기간 경과 후 6개월 상한**이다. 본문에 1년으로 적었다면 그것이 이 브리프의 가장 큰 오류가 됐을 자리다. ⓓ·ⓔ **둘 다 맞았다.** 국내 고지가 본문에서 特措法 제3조3항 연장 고지를 별도 문서로 링크하고, `kumamoto2026_sochi`는 각국·지역 지재청의 구제조치(일본 출원인의 해외 방향)로 방향이 반대다. 다섯 계열(국내 고지 / 特措法 연장 / 각국 지재청 / 특별조치 일람 / 상설 틀)을 본문에서 섞지 않는다. trigger의 `2026-08-07 갱신`도 정확하다 — 국내 고지 본문 머리와 `[更新日 2026年8月7日]`가 그 날짜다. 지정기간(거절이유통지 응답기간)은 의장·상표 심사에서 기간 도과 후 제출을 인정하며 意見書로 신청한다. 발행 여부와 법률 사실의 최종 판단은 계속 owner 몫이다."
  },
  {
    ...
    status: "watching",
    notes:
      "2026-09-12 sweep에서 EUIPO 뉴스 목록을 열어 확인했다(등록 이후 첫 실사). 목록 수준에서 제목·날짜·요약만 대조했고 **상세 고지 본문은 아직 열지 않았다** — 무엇이 어떻게 바뀌었는지(연장 횟수·중지 요건·기산 규칙)를 본문으로 확인하기 전에는 EuTm 제8장 일정 서술에 닿는지 판단할 수 없다. 그게 발행 전 첫 작업이다. 같은 목록에서 2026-09-09 「Security alert: Beware of fraudulent websites impersonating the EUIPO」도 관측됐다 — 사칭 사이트 경고는 UKTm·EuTm이 이미 다루는 미청구 납부요구(unsolicited payment request) 축과 같은 계열이라 단독 브리프보다 기존 본문 보강 쪽이 맞다고 보고 후보로 올리지 않는다. **2026-09-13 WebSearch triage(상세 고지 본문 여전히 미열람)**: 심판부 쪽은 Rules of Procedure 개정으로 기간 산정·연장(제3조)과 중지(제44조)가 2024-03-01 시행됐고 이후 연 1회 개정 사이클이 도는 것으로 검색됐다. 2026-07-17 고지가 말하는 `1심과 2심 정렬`은 그 심판부 기준선에 1심을 맞춘 것이라는 읽기와 정합하지만, 무엇이 어떻게 바뀌었는지는 여전히 고지 본문에서만 확인된다. 세 후보 중 trigger가 가장 오래됐다(2026-07-17)."
  },
  {
...
export const briefSweepLog: BriefSweep[] = [
  {
    ...
    foundCandidateIds: [],
    note:
      "신규 발굴 회차가 아니라 `2026-09-jpo-disaster-deadline-relief`의 발행 전 확인 항목을 닫은 회차다. "
      + "이 세션의 채널은 등록 소스 17개 전건 `connect_rejected`였고(`sweep 절차` 0번 실측), 대조는 다른 채널이 "
      + "떠 온 응답 바이트 캡처로 했다 — 아카이브 sha256 `a2d564d52b7648b6…b0cd367f`, 캡처 2026-09-13 08:54~08:55Z, "
      + "25건 전건 해시·바이트 재계산 일치. 규칙은 `docs/briefs-discovery.md` `캡처 근거 대조`. "
      + "연 것은 jpo의 災害関連情報 계열 10건(索引·국내 고지 20260729/갱신 20260807·特措法 연장 20260807·특별조치 일람·"
      + "각국 지재청 구제·상설 틀 kihon/method2/riyu·구제 가이드라인 PDF 2건)이며, `sweepTarget`이 가리키는 報道発表 "
      + "목록은 이번에 열지 않았다 — 2026-08-30 회차가 그 목록만 보고 이 소재를 놓쳤던 지점이라 계열을 명시해 둔다. "
      + "산출은 신규 후보 0건이고, 기존 후보 하나가 `watching → ready`로 올라갔다.",
  },
  {
    sweptOn: "2026-09-12",
    kind: "verified",
