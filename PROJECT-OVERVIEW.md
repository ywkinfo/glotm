...
| Current focus | 루트 GloTm 셸 기준 LatTm 읽기 경험, 7개 live guide 회귀 유지, Gateway 포트폴리오 정합화, 콘텐츠 QA, 로컬 빌드 안정화 |
...
- 현재 개발된 포트폴리오는 총 7개 가이드이며, 모두 루트 셸에서 직접 열 수 있다. 다만 `UKTm`은 draft 공개본 성격의 early track으로 취급한다.
- `LatTm`: 20개 챕터, 검색 엔트리 780개 생성 확인. 여전히 Phase 1의 주 개선 대상이다.
- `MexTm`: 15개 챕터, 검색 엔트리 266개 생성 확인. 홈/챕터/검색 외에 continue reading, outline, 진행률, 이전/다음, 섹션 추적까지 core reader parity 범위로 맞춘다.
- `UsaTm`: 14개 챕터, 검색 엔트리 165개 생성 확인. 루트 셸 `/usa` 경로에 연결된 live country guide다.
- `JapTm`: 15개 챕터, 검색 엔트리 75개 생성 확인. 루트 셸 `/japan` 경로에 연결된 live country guide다.
- `ChaTm`: 15개 챕터, 검색 엔트리 129개 생성 확인. 루트 셸 `/china` 경로에 연결된 live country guide다.
- `EuTm`: 14개 챕터, 검색 엔트리 180개 생성 확인. 루트 셸 `/europe` 경로에 연결된 live regional guide다.
- `UKTm`: 14개 챕터, 검색 엔트리 84개 생성 확인. 루트 셸 `/uk` 경로에 연결된 live country guide이며, 현재는 draft 공개본 성격의 early track이다.
...
글로벌 시장 전체를 지향하되, 현재는 7개의 live shell guide를 함께 운영 중인 상표 운영 가이드 플랫폼.
...
GloTm의 최종 사업 범위는 특정 지역이 아니라 **글로벌 시장 전체**다. 현재는 `LatTm` 기준 제품을 중심축으로 유지하되, 이미 개발된 포트폴리오는 `LatTm`, `MexTm`, `UsaTm`, `JapTm`, `ChaTm`, `EuTm`, `UKTm`까지 확장되어 있다. 다만 실행 우선순위는 여전히 `LatTm` 중심 Phase 1 안정화이며, 멕시코는 가장 중요한 우선 심화 시장으로 보고, `UKTm`은 draft 공개본 성격의 early track으로 함께 운영한다.
...
현재 개발된 포트폴리오는 권역형 2개(`LatTm`, `EuTm`)와 단일국가형 5개(`MexTm`, `UsaTm`, `JapTm`, `ChaTm`, `UKTm`)로 구성되며, 현재는 7개 모두 루트 셸에서 직접 읽을 수 있다. 다만 실행 우선순위는 계속 `LatTm` 중심이며, `UKTm`은 draft 공개본 성격의 early track으로 약하게 노출한다. 현재는 바로 배포하기보다 로컬 웹앱으로 충분한 완성도를 확보한 뒤 파일럿 배포로 넘어가는 순서를 계획한다.
...
| `ChaTm` | 단일국가 | live shell | 15 | 129 | 중국 live country guide |
| `EuTm` | 권역형 | live shell | 14 | 180 | 유럽 live regional guide |
| `UKTm` | 단일국가 | live shell | 14 | 84 | 영국 early track |
...
`MexTm`은 전략적으로 중요한 멕시코 심화 가이드다. 현재 `npm run build`는 통과하며 `document-data.json` 기준 15개 챕터와 266개 검색 엔트리를 생성한다. 리더 측면에서는 continue reading, outline, reading progress, 이전/다음 이동, 섹션 추적, action bar까지 루트 셸 기준 core reader parity를 맞췄다. 다만 `manifest.json`, `build-master.ts`, `qa-content.ts`가 없어 LatTm 수준의 원고 조립·QA 파이프라인은 아직 갖춰지지 않았다.
...
`ChaTm`은 중국 단일 시장 상표 실무를 다루는 가이드다. 현재 `document-data.json` 기준 15개 챕터와 129개 검색 엔트리를 생성하며, 루트 `GloTm` 셸 `/china` 경로에 연결된 상태다. `LatTm` 수준의 `manifest.json`, `build-master.ts`, `qa-content.ts`는 아직 없지만, 런타임 읽기 경로는 live shell에 포함됐고 카드에서는 "지속 업데이트 중" 상태를 함께 노출한다.
