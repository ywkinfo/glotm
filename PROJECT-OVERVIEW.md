...
| Current phase | Phase 1 — GloTm 셸 기준 공통 콘텐츠 파이프라인 정규화 + live shell portfolio alignment |
| Current focus | 루트 검증 기준 복구, 6개 live guide 공통 QA 정착, Gateway 포트폴리오 메타데이터 정합화, 로컬 빌드 안정화 |
| Do not start yet | prerender/정적 메타데이터 준비, 이메일 게이트, 글로벌 확장 템플릿화 |

### 현재 운영 스냅샷

- 루트 `GloTm`: `npm run build` 통과. `BrowserRouter` 기반 셸에서 Gateway, `LatTm`, `MexTm`, `UsaTm`, `JapTm`, `ChaTm`, `EuTm`를 함께 연결한다.
- 런타임 포트폴리오 메타데이터의 기준 파일은 `src/products/registry.ts`다. 제품 상태, 챕터 수, 검색 엔트리 수, 성숙도 메모는 이 파일을 먼저 갱신한다.
...
| `MexTm` | 단일국가 | live shell | 15 | 266 | 멕시코 심화 트랙 |
| `UsaTm` | 단일국가 | live shell | 14 | 165 | 미국 live country guide |
| `JapTm` | 단일국가 | live shell | 15 | 75 | 일본 live country guide |
| `ChaTm` | 단일국가 | live shell | 6 | 32 | 중국 live country guide |
...
### MexTm (멕시코 상표 실무 가이드) — 파일럿 2단계 핵심 트랙, core reader parity + 공통 파이프라인 정착 완료

`MexTm`은 전략적으로 중요한 멕시코 심화 가이드다. 현재 `document-data.json` 기준 15개 챕터와 266개 검색 엔트리를 생성한다. 리더 측면에서는 continue reading, outline, reading progress, 이전/다음 이동, 섹션 추적, action bar까지 루트 셸 기준 core reader parity를 맞췄고, 콘텐츠 측면에서는 `manifest.json`, `build-master.ts`, `qa-content.ts`를 포함한 공통 조립·QA 파이프라인을 갖췄다.
...
`ChaTm`은 중국 단일 시장 상표 실무를 다루는 가이드다. 현재 `document-data.json` 기준 6개 챕터와 32개 검색 엔트리를 생성하며, 루트 `GloTm` 셸 `/china` 경로에 연결된 상태다. 공통 조립·QA 파이프라인은 정착했고, 최근 실무 섹션 보강으로 QA 경고도 제거했다. 다만 포트폴리오 안에서는 여전히 가장 얇은 가이드이므로 사실 검증 로그 보강과 본문 심화가 다음 우선순위다.
...
| ChaTm 생성 산출물 | 6개 챕터 / 검색 엔트리 32개 |
| EuTm 생성 산출물 | 14개 챕터 / 검색 엔트리 180개 |
| GloTm build status | `npm test`, `npm run build`, `npm run build:pages:glotm` 결과로만 갱신 |
| Workspace content status | `npm run content:prepare` 결과로만 갱신 |
| MexTm 현재 한계 | fact verification log와 심화 본문을 계속 보강해야 함 |
...
### Phase 4: MexTm 콘텐츠 확장 및 검증 고도화 (멕시코 심화 파일럿)

| 작업 | 상세 |
|------|------|
| 본문 심화 | IMPI 운영, 집행, 국경조치, 거래 구조 장의 밀도 확대 |
| fact verification | `content/research/mx_tm_fact_verification_log.md` 지속 갱신 |
