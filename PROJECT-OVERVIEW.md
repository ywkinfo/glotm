...
- `MexTm`: 15개 챕터, 검색 엔트리 292개. `growth` tier.
- `ChaTm`: 15개 챕터, 검색 엔트리 197개. `growth` tier.
...
| `MexTm` | 단일국가 | growth | mature | 15 | 292 | buyer entry 핵심 트랙 |
| `ChaTm` | 단일국가 | growth | beta | 15 | 197 | 중국 실무 밀도 강화 트랙 |
...
`MexTm`은 전략적으로 중요한 멕시코 심화 가이드다. 현재 `npm run build`는 통과하며 `document-data.json` 기준 15개 챕터와 292개 검색 엔트리를 생성한다. 리더 측면에서는 continue reading, outline, reading progress, 이전/다음 이동, 섹션 추적, action bar까지 루트 셸 기준 core reader parity를 맞췄다. 워크스페이스 자체에는 `manifest.json`, `build-master.ts`, `qa-content.ts`가 갖춰져 있으며, 루트 `content:prepare`에서도 `build-master -> qa-content -> build-content` 전체 흐름을 사용한다.
...
`ChaTm`은 중국 단일 시장 상표 실무를 다루는 가이드다. 현재 `document-data.json` 기준 15개 챕터와 197개 검색 엔트리를 생성하며, 루트 `GloTm` 셸 `/china` 경로에 연결된 상태다. 현재 루트 `content:prepare`에서도 `build-master.ts -> qa-content.ts -> build-content.ts` 전체 흐름을 타며, 카드에서는 "Sprint 1 잠금 6장 보강 완료" 상태를 함께 노출한다.
...
| MexTm 생성 산출물 | 15개 챕터 / 검색 엔트리 292개 |
| UsaTm 생성 산출물 | 14개 챕터 / 검색 엔트리 171개 |
| JapTm 생성 산출물 | 15개 챕터 / 검색 엔트리 140개 |
| ChaTm 생성 산출물 | 15개 챕터 / 검색 엔트리 197개 |
