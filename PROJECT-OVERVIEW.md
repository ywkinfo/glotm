...
- `JapTm`: 15개 챕터, 검색 엔트리 100개. `incubate` tier.
...
| `JapTm` | 단일국가 | incubate | pilot | 15 | 100 | lighter track |
...
`JapTm`은 일본 단일 시장 상표 실무를 다루는 가이드다. 현재 `document-data.json` 기준 15개 챕터와 100개 검색 엔트리를 생성하며, 루트 `GloTm` 셸 `/japan` 경로에 연결된 상태다. 이번 verification refresh로 local full pipeline과 fact review를 다시 통과했고, lifecycle은 `pilot`로 정렬했다. search density도 추가 보강으로 올라왔지만 아직 beta 기준에는 못 미치므로, 다음 우선순위는 대형 확장보다 density와 reader-facing utility를 더 끌어올리는 일이다.
...
| JapTm 생성 산출물 | 15개 챕터 / 검색 엔트리 100개 |
