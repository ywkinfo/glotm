...
- guide별 병렬 작업이 필요할 때는 Codex native parallel subagents를 기본 수단으로 사용한다.
- 이 저장소에서는 `omx team` / tmux worker orchestration을 기본 병렬 경로로 쓰지 않는다.
- 각 workspace lane은 가능한 한 local/workspace evidence를 먼저 잠그고, 루트 `health:*` lane은 리더가 shared root gate로 1회 실행한다.
...
- workspace priority lane은 개별적으로 루트 gate를 반복하기보다 lane-local evidence를 먼저 만든다.
- shared root gate(`health:runtime` -> `health:content` -> `health:release`)는 리더 통합 단계에서 1회 실행하는 것을 기본으로 한다.
