...
작성: 2026-09-15 (18라운드) · 갱신: 2026-09-16 (20라운드 — 조문 채널이 열려 큐 10건을 닫았다, §5)
...
20라운드가 10건을 닫아 **16건**이 남았다. 남은 16건은 A류 5건(의도된 단일 소스)과 C류 11건이다.

| 워크스페이스 | HIGH | 단일 소스 (09-15) | 단일 소스 (09-16) | 비중 |
|---|---:|---:|---:|---:|
| JapTm | 8 | 7 | 1 | 12% |
| UKTm | 7 | 5 | 4 | 57% |
| EuTm | 9 | 6 | 5 | 56% |
...
09-16 열이 줄어든 자리는 전부 **1차 법령을 붙여서** 줄었다. 안내면을 하나 더 얹어 개수를 채운 자리는 없다 — §4가 경계하는 게 그것이다.
...
## 2. B류 — 회차가 조문을 이미 특정했는데 register에 없다 (6건 · **2026-09-16 전건 닫음**)
...
|---|---|---|---|
| `CN-EVD-001` | `cnipa-nonuse-cancellation-guide-2023` | **商标法实施条例 제66조** (notes: "2개월 사용증거 window는 …실시조례 근거로, 법률 본문이 아님") | **닫힘(09-16)** — 국가행정법규고 정본으로 제66조 축자 확인, `cn-trademark-law-implementing-regulations`(+ `-article-66`) 신설 |
| `UK-RENEW-001` | `ukipo-renew` | **TMA 1994 §40(3)** (notes: 갱신 10년 기산이 등록완료일이 아니라 출원일인 근거) | **닫힘(09-16)** — §40(3)·§42 축자 확인, `uk-tma-1994`(+ `-section-40`·`-section-42`) 신설 |
| `JP-REP-001` | `jpo-step-by-step` | **特許法 제8조** (notes가 JPO 원문을 축자 인용: `must be appointed as a "Patent Administrator" (Article 8, Patent Act)`) | **닫힘(09-16)** — e-Gov 정본으로 제8조 확인, `jpn-patent-act`(+ 영문 `-en`) 신설 |
| `JP-MADRID-REFUSAL-001` | `jpo-madrid-faq` | **特許法 제8조** + 통지 발송일 기산 3개월 | **닫힘(09-16)** — 제8조만 연결. 3개월은 조문이 아니라 통지 기재라 이번에 확정하지 않았다 |
| `EU-ENF-001` | `ec-customs-defend-your-rights` | **Regulation (EU) No 608/2013** (notes: "여전히 유효 기준") | **닫힘(09-16)** — EUR-Lex 본문에서 제2조 제10호·제11호 확인(`In force`), `eu-reg-608-2013` 신설 |
| `LA-CL-NONUSE-001` | `inapi-chile-ley-21355` | **Ley 21.355** | **닫힘(09-16)** — BCN Ley Chile 정본에서 제27조의2 A 신설 조항 확인, `cl-ley-21355` 신설 |
...
**2026-09-16 닫음.** 제66조 제1항을 축자 확인했다 — 「商标局受理后应当通知商标注册人，限其自收到通知之日起2个月内提交该商标在
...
## 3. C류 — 조문 앵커가 아직 특정되지 않았다 (15건 → **11건**, JapTm 4건 09-16 닫음)
...
|---|---|---|---|
| ~~`JP-FIRST-001`~~ | `jpo-faq-trademark` | 商標法 **제8조 제1항** | **닫힘(09-16)** — 축자 확인 후 `jpn-trademark-act` 연결 |
| ~~`JP-TERM-001`~~ | `jpo-outline` | 商標法 **제19조 제1항·제2항** | **닫힘(09-16)** — 축자 확인 후 `jpn-trademark-act` 연결 |
| ~~`JP-OPP-001`~~ | `jpo-faq-trademark` | 商標法 **제43조의2** | **닫힘(09-16)** — 축자 확인 후 `jpn-trademark-act` 연결 |
| ~~`JP-CUSTOMS-001`~~ | `japan-customs-rightholders` | 関税法 **제69조의13·제69조의4** | **닫힘(09-16)** — 축자 확인. `jpn-customs-act`를 영문 번역본에서 e-Gov 정본으로 교체 |
...
### JapTm이 유독 높았던 이유 — 그리고 09-16에 무엇이 남았나
...
닫힌다 — 비용 대비 회수가 가장 큰 구간"이라고 적었고, 09-16에 실제로 그렇게 닫혔다. JapTm 단일 소스는 **7건 → 1건**이 됐다.
...
## 5. 채널 실측 — 18라운드가 못 한 것, 20라운드가 연 것
...
**20라운드(09-16)는 브라우저 채널로 열었다.** 403은 사이트가 아니라 채널의 문제라는 2026-08-15 결론이 다시
...
> **날짜 기준 — 처음 초안은 KST로 적었고, UTC로 통일했다.** 이 회차는 KST 2026-09-17에 돌았고 그 시각
> UTC는 2026-09-16이다. 초안은 `lastVerified` 10건과 이 문서를 KST 날짜로 적었는데, **저장소의 날짜 계산은
> 전부 UTC다**(`startOfUtcDay`·`elapsedUtcDays`). 그대로 두면 열 건이 UTC 기준으로 하루 미래가 되고,
> `getClaimFreshnessDays`가 `Math.max(0, …)`으로 클램프하기 때문에 **freshness 0으로 읽히며 어떤 게이트도
> 울지 않는다** — 하루 더 신선해 보이는 값이 조용히 통과한다.
>
> 그래서 스탬프와 이 문서의 표기를 **2026-09-16(UTC)** 으로 맞추고, 같은 일이 다시 일어나지 않도록
