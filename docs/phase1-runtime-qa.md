...
- `npm run content:prepare`: 워크스페이스별 `build-master -> qa-content -> build-content`
...
### Skeleton 가이드 추가 점검 (UsaTm·JapTm·ChaTm·EuTm)
12. 각 가이드 홈(`/usa`, `/japan`, `/china`, `/europe`)이 빈 화면 없이 챕터 목록을 표시한다.
...
```
- qa-content.ts → build-content.ts 순 실행
- 검증 항목: manifest 15개 장 일치, H2 순서, 금지 마커, 코드 펜스, 표 일관성, 중복 문단, 챕터 깊이 경고

### 나머지 가이드 (build-content.ts 단독)
```
npm run content:usa
...
```
- 현재 구조 검증 없음 — 콘텐츠 보강 후 manifest + qa-content 파이프라인 순차 추가 예정
