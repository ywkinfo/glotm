...
- 루트 health 계약은 `health:runtime`, `health:content`, `health:release` 세 lane으로 나눈다.
- 제품 상태, 챕터 수, 검색 엔트리 수, 성숙도 메모는 `src/products/registry.ts`를 기준으로 본다.
- 자동 검증과 수동 스모크의 경계를 명확히 유지한다. 자동 검증이 통과해도 아래 수동 항목은 별도로 확인한다.
- 월간 scorecard review에서는 `verificationFreshnessDays`, `qaLevel`, `highRiskVerificationGapCount`를 health report 입력값으로 본다.
...
- `npm run health:runtime`: `typecheck`, `test:runtime`, `npm run e2e:smoke`를 묶어 셸 계약, 리더 계약, 콘텐츠 링크 계약, 실제 브라우저 스모크를 검증한다. generated-content 의존 회귀 테스트는 이 lane에서 제외해 pure runtime check로 유지한다.
- `npm run health:content`: 루트 `content:prepare`와 `test:content`를 실행한 뒤 `ChaTm`·`MexTm`·`EuTm` workspace local `content:prepare`까지 재현한다. `UsaTm`·`JapTm`·`UKTm`은 lighter-track 기준으로 루트 refresh 계약을 따른다.
- `npm run health:release`: `npm run build`, `npm run build:pages:glotm`를 묶어 루트 셸 빌드와 GitHub Pages subpath 배포 경로를 검증한다.
- `npm run health:report`: 루트 lane 결과와 product scorecard 메타데이터를 같은 포맷으로 정리한다.
...
`ChaTm` Sprint 1 잠금 6장(제2장, 제3장, 제5장, 제6장, 제7장, 제10장)을 보강한 뒤에는 아래 항목을 수동으로 다시 본다.
...
`MexTm` 잠금 4장(제1장, 제4장, 제11장, 제13장)을 보강한 뒤에는 아래 항목을 수동으로 다시 본다.

1. `/mexico` 홈에서 잠금 4장이 chapter card 목록과 continue reading 추천 흐름에서 정상 노출된다.
2. 제1장에서 IMPI system map, 공식 시스템명 표준안, 첫 30일 체크리스트가 표 깨짐 없이 보인다.
3. 제4장에서 직접출원 vs 마드리드 비교표, buyer-entry 경로 선택표, 기한·비용 handoff 표가 search와 outline에 잡힌다.
4. 제11장에서 통제 포트폴리오 보드와 quick triage 표가 모바일에서도 읽을 수 있다.
5. 제13장에서 사례 회수 decision table, 실패 패턴, 예방 기준표가 서로 중복 없이 이어진다.
6. `/mexico` 검색에서 잠금 4장의 decision table·체크리스트 항목이 섹션 단위로 탐색된다.
7. `/mexico` continue reading이 장 중간 deep link로 복귀할 때 scroll/outline contract가 깨지지 않는다.
