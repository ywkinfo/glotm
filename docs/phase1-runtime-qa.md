...
- `npm run content:prepare`: 루트 검증 계약용 mixed pipeline 실행. 현재 `LatTm`·`ChaTm`·`EuTm`·`UKTm`은 `build-master -> qa-content -> build-content` 전체 흐름을 타고, `MexTm`·`UsaTm`·`JapTm`은 `build-content.ts` shortcut으로 generated JSON을 갱신한다.
...
### MexTm (QA 파이프라인 추가 완료)
```
npm run content:mexico
```
- 루트 검증 계약에서는 현재 `build-content.ts` shortcut으로 generated JSON을 갱신한다.
- 워크스페이스 로컬에서는 `cd MexTm && npm run content:prepare`로 `build-master.ts -> qa-content.ts -> build-content.ts` 전체 흐름을 실행할 수 있다.
- 구조 수정 시에는 로컬 `content:prepare`를 우선 검증 경로로 사용한다.
