...
- 워크스페이스 로컬 `content:prepare`는 루트 refresh와 동일한 full pipeline이며, deeper 편집·QA 시 기준 경로로 본다.
- 현재 7개 가이드 모두 루트 `content:prepare`에서 full pipeline(`build-master → qa-content → build-content`)을 사용한다. `health:content`의 workspace-local 재현은 `ChaTm`·`MexTm`·`EuTm`·`UsaTm`·`JapTm`·`UKTm` 여섯 개를 포함하며, `LatTm`만 루트 full pipeline으로 검증하고 workspace-local 추가 실행은 포함하지 않는다.
...
| `JapTm` | full (`build-master → qa-content → build-content`) | ✓ `npm --prefix JapTm run content:prepare` | 동일 |
| `UKTm` | full (`build-master → qa-content → build-content`) | ✓ `npm --prefix UKTm run content:prepare` | 동일 |

**그룹 요약:**
- **Growth group** (MexTm, ChaTm, EuTm, UsaTm, JapTm, UKTm): root full pipeline + health:content workspace-local 재현 — 가장 깊은 검증
- **Flagship baseline** (LatTm): root full pipeline은 타지만 health:content workspace-local 추가 실행에는 포함하지 않는다 — 루트 검증만 재현

`JapTm`(2026-06-30 #114)과 `UKTm`(2026-07-07 #121) growth/mature 승급 이후 두 가이드 모두 root full pipeline과 `health:content` workspace-local 재현을 함께 사용한다. 과거 `JapTm` shortcut·`UKTm` incubate/beta 표기는 승급 이전 잔재다.
...
npm run content:uk
```
- `UsaTm`, `JapTm`, `ChaTm`, `EuTm`, `UKTm`: 루트 검증 계약에서도 `build-master → qa-content → build-content` 전체 흐름을 사용하고, `health:content`의 workspace-local 추가 실행에도 모두 포함된다(`MexTm`도 동일). 콘텐츠를 직접 수정하거나 deeper content QA가 필요할 때는 워크스페이스 로컬 `content:prepare`를 직접 실행한다.
- `LatTm`: 루트 `content:prepare`에서 full pipeline을 타지만 `health:content`의 workspace-local 추가 실행에는 포함하지 않으므로, deeper content QA는 `npm --prefix LatTm run content:prepare`로 별도 실행한다.
