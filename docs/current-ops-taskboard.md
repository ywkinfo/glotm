...
- 2026-08-02 미해결로 남긴 것(리뷰에서 실측 확인, 별도 라운드 필요): ① ~~sitemap `lastmod` 145건 중 **121건이 빌드 타임스탬프** — LatTm 콘텐츠 최종 변경 2026-06-23·JapTm 2026-07-01인데 둘 다 배포 시각을 신고해, 배포마다 전 코퍼스가 갱신됐다고 거짓 신호를 낸다.~~ → **2026-08-08 해소(위 라운드)**. ② 라이브 `<title>` 중복 4클러스터 10건(`서문 | GloTm` 4건은 관할 구분 없음), `description` 7건이 동일 placeholder `도입 MexTm 가이드 챕터.`. ③ **claim staleness가 어떤 게이트도 붉히지 못한다** — `audit:facts`·`check:consistency`는 어느 워크플로에도 없고(ci.yml은 health 3레인만), `audit-staleness.ts`는 `level: "warning"`이라 exit 0이며, `health-report.test.ts`는 시계를 2026-06-10에·`scorecard.test.ts`는 2026-07-20에 고정해 실제 경과일이 평가되지 않는다. EuTm claim 10건 2026-08-08 만료도 자동으로는 아무도 알리지 않는다. ④ `factual-qa-rollout.md` 18·57·365행이 "JapTm은 root shortcut-refresh 예외"라 단정하나 `content:japan`은 full pipeline이다.
