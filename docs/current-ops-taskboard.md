...
- 2026-08-30 라운드: **9월 만료 claim 3벌을 실제로 재대조하고 브리프 소스 sweep을 돌렸다 — 직전 라운드의 채널 전제가 다시 틀렸기 때문이다.** 8-25 후속 라운드는 "`curl`·WebFetch는 등록 소스 15개 전부 CONNECT 403"이라 적었으나, 이번 세션에서 실측하면 **등록 소스 97개 중 ~85개가 curl 200**이고 JPO·EUIPO는 브라우저 UA를 붙이면 열리며 `sbj.cnipa.gov.cn`은 curl 403이어도 **인앱 브라우저로 본문 전체가 열린다**. `briefs-discovery.md`가 이미 이 경우를 계약에 써 뒀다 — `lastVerified` 갱신은 "owner 또는 **그 채널이 열리는 세션**"의 몫이다. 그래서 이번 라운드는 triage가 아니라 **재대조**다.
  - **결과: china 8/8·uk 11/11·mexico 4/4 값 변경 없음.** `lastVerified`를 2026-08-30으로 갱신하고 registry `factsReviewedOn`을 china·mexico·uk 모두 2026-08-30으로 re-stamp했다(`verifiedOn`은 lane freshness라 건드리지 않았다). 창 마감 9/19–9/20 전에 종결.
  - **UK-IPEC-001 owner 액션 종결 + 8-25 메모 3군데 정정.** 규범을 확정했다: 근거는 `CPR Practice Direction 45/46`이 아니라 **CPR Part 46 Section VII rule 46.21(1)**이다(PD가 아니라 rule, Part 45가 아니라 46 — 2023년 Part 45 전면 개편 후 그 Part에는 IPEC 절이 없다). damages 상한 £500,000의 근거도 Part 46이 아니라 **CPR 63.17A(1)**이었다. 그리고 8-25가 경고한 "GOV.UK·judiciary.uk가 개정 이전 £50,000을 싣고 있다"는 **현행 안내면에서 재현되지 않았다** — £50,000이 남은 곳은 2013·2015년 GOV.UK 보도자료 3건뿐이고, 그 URL을 fact log에 적어 다음 재검증자가 걸리지 않게 했다. 실제로 메운 갭은 안내면이 규율하지 못하는 셋이다: quantum £30,000, **CPR 46.20(2)(b)**(유효성 certified된 등록상표 사건 적용 제외), **CPR 63.27–63.28**(small claims track에는 scale costs 미적용). 뒤 둘은 Ch10·Ch13 포럼 선택 표 반영 후보다. `UK-IPEC-001.sourceIds` 2건 → 7건.
  - **죽은 출처 3건 교체.** ⓐ `UK-IR-001`: Gibraltar 자동 효력이 등록 2면 어디에도 없었다(`gov-protect-abroad`가 2026-06-04 전면 재작성되며 빠졌다) → GOV.UK 「Extension of UK IP rights abroad — countries C to H」 §6을 열어 등록. ⓑ 리다이렉트 2건(`objecting-to-other-peoples-trade-marks`, WIPO uk DRS)을 최종 URL로 교체. ⓒ **`MX-FEE-001`·`MX-DL-001`**: gob.mx 포털 개편으로 `/tramites/ficha/*` 네임스페이스가 통째로 사라져 IMPI88·IMPI3170이 죽었다. 값이 아니라 근거가 안내면에 걸려 있던 것이 문제이므로 **LFPPI 조문으로 교체**했다 — 제233조(허여 3주년 후 3개월 단독 사용선언, 미선언 시 `caducará de pleno derecho`)와 제237조(`Al presentar la solicitud de renovación, el titular deberá declarar el uso real y efectivo`)가 `IMPI-00-002`와 `IMPI-00-014`가 왜 별개 서식인지를 직접 설명한다. **ficha ID 자체는 확인 못 했다** — CONAMER Catálogo Nacional은 이 채널에서 검색 결과를 반환하지 않고 IMPI formatos 면은 텍스트 없는 스캔 PDF뿐이라, claim에서 ficha 축을 유지할지는 owner 판단으로 남긴다.
  - **채널 함정 2건(기록 필수).** ① 죽은 gob.mx ficha URL은 `curl`에 **HTTP 200**을 준다 — 본문은 1,907바이트짜리 `Challenge Validation` WAF 응답이다. `gob.mx/impi/documentos/*` 계열도 같다(22자 응답). **200을 도달 증거로 쓰면 안 된다.** ② 반대로 403도 도달 불가의 증거가 아니다(브라우저 UA·인앱 브라우저로 열린다). 두 방향 모두 본문을 열어 확인해야 한다.
  - **브리프 sweep 1회차(`verified`) 기록 — 5개 소스.** 등록 15개 중 14개가 "실사 이력 없음"이던 상태에서 kipo·wipo-madrid·jpo·cnipa-official·cnipa-trademark-office를 실제로 열었다(→ never-verified 14 → 10). 산출: ⓐ 지식재산처 2026-08-24 보도자료(게시물 21009)에서 **「2026년 K-브랜드 정부인증제도」 1차 모집 2026-08-24~09-11** 확인 → watching 후보 `2026-08-kbrand-certification-launch`를 **ready**로 승격(참여요건 ①이 "대상국에 기업 자신의 상표가 이미 출원·등록돼 있을 것"이라 가이드 lane과 바로 연결된다). ⓑ WIPO Madrid 2026-07-31 공지에서 **USPTO TEASi → Madrid e-Filing 전환, 2026-10-01 단일 창구** 확인 → 신규 후보 `2026-10-uspto-madrid-efiling-cutover` **ready** 등록(UsaTm은 열린 후보 0건이었다). ⓒ JPO 報道発表 2026년도·CNIPA 개정법 專題·商标局 通知公告(~2026-08-20)는 상표 실무 트리거 없음 — **산출 없음도 기록했다**. `ready` 잔량 **0 → 2**로 다음 발행의 앞단이 채워졌다.
  - **남은 것**: samr·euipo·eu-customs-reform·govuk-ipo·uspto·cbp-ipr·impi·inapi-chile·sic-colombia·inpi-argentina 10개 소스는 이번 회차에서 열지 않았다(freshness 미갱신). 런북 §1 SC·§2 GA4·§4 organic sessions는 여전히 owner 전용이다.

- 2026-08-30 후속 라운드: **`audit:facts`가 못 보던 sourceId 34개를 닫았다.** `audit:facts`는 HIGH claim에 sourceId가 **몇 개 있는지**만 세므로(`scripts/research-audit/audit-facts.ts:8`) 실재하지 않는 id를 써도 `factIntegrity=100`이 나온다. 회귀 가드는 있었지만 `workspaces` 배열에 JapTm·UsaTm 2개만 하드코딩돼 있어 나머지를 못 봤다. 앞 라운드에서 MexTm의 죽은 ficha 출처를 게이트가 끝까지 통과시킨 것이 같은 빈틈이다.
  - **가드를 목록에서 발견으로 바꿨다.** `claim-source-register.test.ts`가 이제 claim-map을 가진 워크스페이스를 **파일시스템에서 찾아** 전부 검사하고, register가 없으면 그 자체로 실패한다(`*_source_register.md` 규약). 구멍이 생긴 원인이 "손으로 유지하는 목록"이었으므로 목록을 없앤 것이다. 테스트 19 → 28건.
  - **ChaTm·MexTm source register 신설.** ChaTm 14건·MexTm 10건 전부 본문을 열어 축자 인용과 함께 확정했다. URL은 대부분 fact log의 `official_source` 열에 이미 있었다. ChaTm은 `cnipa-trademark-law`·`-article-33`·`-article-34`가 한 페이지의 조문별 포인터(앵커 없음)라는 점, `cnipa-fee-guide-2019`가 URL만 2019이고 실제로는 2025-11-03 재게시본이라는 점을 표에 적었다.
  - **EuTm 8건은 URL을 붙이지 않고 폐기했다.** `2026-08-02 재검증 라운드` 표의 `결정 근거` 열이 그 8개(`euipo-*-guidance`·`euipo-brexit-qa`·`govuk-eu-trade-mark-protection`)가 **실제 검증 근거가 아니었음**을 보여준다 — 그 라운드는 전건을 EUTMR 조문과 GOV.UK로 대조했고 EUIPO 안내면을 연 회차는 없다. 그래서 `eutmr-consolidated`·`govuk-comparable-uk-marks`로 remap했다. `govuk-eu-trade-mark-protection`은 `govuk-comparable-uk-marks`와 **같은 URL을 가리키던 중복 id**였다. **열어본 적 없는 URL을 그럴듯하게 붙이면 추적 가능성의 외형만 만들어진다** — 그 빈틈을 메우려고 만든 가드에 같은 빈틈을 새로 넣는 셈이다.
  - **채널 실측 2건 추가.** ⓐ `gob.mx` HTML은 **살아 있어도 curl로 안 열린다** — UA를 붙여도 200 + 약 1,900바이트 `Challenge Validation`이 오고, 이 응답은 죽은 페이지와 산 페이지에서 **똑같이** 나온다(삶/죽음은 브라우저에서만 갈린다). 단 `gob.mx/cms/uploads/*` 정적 첨부는 통과한다. ⓑ `publications.europa.eu` CELEX 경로는 register 접근 메모대로 `Accept: application/xhtml+xml`만 붙이면 **HTTP 400**이다 — `Accept-Language: eng`가 함께 필요하다. 메모를 고쳤다.
  - **앞 라운드 메모 1건 정정.** MexTm fact log의 "IMPI formatos 면은 스캔 PDF만 싣는다"는 **틀린 페이지를 본 것**이었다. `impi.gob.mx/formatos`는 홍보 자산 폴더이고, 서식 코드 정본은 `gob.mx/impi/acciones-y-programas/servicios-que-ofrece-el-impi-formatos`에 HTML 표로 있으며 거기서 `IMPI-00-002`·`IMPI-00-014`를 직접 확인했다. **MX-FEE-001의 서식 축 갭은 닫혔고** ficha ID 축만 owner 판단으로 남는다.
  - **남은 것**: ChaTm의 `sphfwfl` 상설 코너에 대응하는 sourceId 부재, `CN-NORM-001`의 SAMR sourcing gap, MexTm ficha ID 존속 여부, `www.euipo.europa.eu` 채널(curl·브라우저 모두 본문 미렌더). 게이트: `health:all` green(293 unit + e2e 28, content 44, seo 22), `audit:facts` 6/6 pass, `check:consistency` 0/0.

- 2026-08-31 라운드: **`check:consistency`가 구조적으로 못 보는 구간의 문서 drift 24건 정합.** 체커는 registry ↔ `PROJECT-OVERVIEW`/`README`만 보고 `docs/workspaces/**`·워크스페이스 `README`/`Harness`·`docs/*.md`는 보지 않는다 — 이 라운드 내내 체커는 0 hard / 0 advisory였다. 알려진 5건에서 시작해 3개 영역(워크스페이스 문서 21개 / `docs/workspaces` 16개 / 루트+`docs`)을 전수 스캔하고 적대적 검증을 거쳐 **24건 확인 · 3건 반증**했다.
  - **가장 위험했던 것은 수치가 아니라 편집 규칙이었다.** `UKTm/Harness/Architecture.md:42`와 `Content-Spec.md:41`이 "growth/mature 표현을 **피하라**"고 지시하고 있었다(정본은 2026-07-07 #121로 growth/mature/full). 규칙의 방향을 뒤집어 이제 승급 이전 라벨을 현재형으로 쓰지 말라고 지시한다. **그리고 그 규칙의 상류가 따로 있었다** — `docs/workspaces/UKTm/uktm-content-expansion-plan.md:22–23`이 그 세 파일에 early-track·beta/standard를 명시하라고 지시하는 원본이다. 이걸 안 고쳤으면 다음 편집자가 harness를 되돌렸을 것이다. 같은 파일의 superseded 배너(:3)도 정본을 `beta / standard`라고 단정하고 있어 **배너 자체가 두 단계 뒤처져** 아래 본문 전체를 dated 처리하지 못하고 있었다.
  - **stale 수치**: EuTm 검색 엔트리 `260`(정본 261) 4곳 — 워크스페이스 3곳 + `eutm-root-sync-input.md` — 과 EuTm claim-map `10건`(정본 11) 3곳, UKTm `14장`(정본 15) 2곳, sitemap `146`(정본 147) 3곳, MexTm claim 검증일 `2026-04-01`(정본 2026-08-30), JapTm `2026-03-31`(정본 2026-08-15), taskboard Next Lane의 JapTm·UKTm·UsaTm `factsReviewedOn`/`verifiedOn` 3쌍.
  - **완료된 스프린트를 현재 과제로 읽히게 두던 문장 3건**: `EuTm/Harness/Content-Spec.md`의 "이번 스프린트에서는 15장 체계로의 전환을 우선한다"(#69/#70로 2026-06-10 완료 — 같은 파일 두 줄 위가 그 baseline을 "확정"이라 선언해 자기모순), 같은 파일의 Ch3/6/10/14 보강 "진행한다", `MexTm/README.md`의 "현재 Sprint 2 우선 장"(2026-05-17 완료, 하필 `Current Lane Truth` 섹션 아래).
  - **QA 체크리스트가 lane 구성을 틀리게 적고 있었다.** `docs/phase1-runtime-qa.md:42`가 `health:release`를 `build + build:pages:glotm`으로 설명해 **`check:dist-boundary`와 `test:seo` 두 게이트가 서술에서 빠져 있었다**(실제는 `build:pages:glotm -> check:dist-boundary -> test:seo`). 이대로면 다음 QA 실행자가 게이트 둘을 건너뛴다.
  - **`monthly-review-template.md:52`의 "fact-review cadence 목표는 아직 미설정"도 사실이 아니었다.** mature 60일 창은 `scripts/research-audit/shared.ts`에 이미 잠겨 있고 `audit:staleness`가 advisory로 측정한다. "주기가 없다"로 읽히면 만료 대응 라운드를 놓친다 — 측정되는 창이 있고 게이팅만 안 한다고 고쳤다.
  - **드리프트의 뿌리는 수치 복제다.** LatTm·ChaTm은 harness에서 수치를 단정하지 않고 `registry.ts`로 위임해 drift가 0이었다. 반면 EuTm은 위임 줄을 가지고 있으면서도 baseline 문자열을 3곳에 복제해 260→261이 28일간 살아남았다. Content-Spec에 "registry 파생 스냅샷" 포인터와 이번 경위를 적었다. **다음에 깨질 자리**로 UsaTm·JapTm·ChaTm이 같은 복제 형태를 쓰는 6곳이 스캔에 잡혔다(현재는 정확).
  - **반증 3건은 고치지 않았다.** `eutm-mature-qa-checklist.md`의 260/10건은 승급 커밋(#69) 시점 registry를 git으로 대조한 결과 **당시엔 맞는 값**이었다(승급 증빙 기록). `portfolio-scorecard.md:140`은 사실 단정이 아니라 조건부 금지 규칙이다.
  - **Hermes PAT 마감 문서화**: `마감 2026-07-15`가 미래형으로 남아 있어 **47일 초과** 상태를 헤딩·상태 블록에 명시했다. bounded task가 owner 트리거로만 도는 구조라 만료 실패가 조용히 지나갈 수 있다는 점을 함께 적었다. 회전 자체는 GitHub 계정이 필요해 owner 몫이다.
  - **`content-gap-review.md`는 표를 고치지 않았다** — 상단 `As of 2026-06-08 · dated snapshot` 배너가 있어 EuTm·UsaTm처럼 Update 주석을 다는 것이 이 문서 규약이다. 갭2가 beta 플로어(≥9.0)로 계산한 여유(UKTm 2 / JapTm 10)는 두 가이드의 mature 승급으로 무효가 됐고, 실제 mature 플로어 180 기준 **여유는 UKTm 3 · JapTm 5**다(`scorecard.ts`로 계산 확인).

- 2026-08-31 후속 라운드: **크롤 표면의 링크 고립섬과 데스크톱 사이드바 접근성 결함 2건.** 둘 다 로컬 게이트가 green인 채로 살아 있던 결함이고, 라이브 실측으로만 드러났다.
...
- 2026-08-31 3라운드: **배포를 CI 성공 뒤로 게이팅.** 그전에는 `CI`·`Verify Release`·`Deploy GitHub Pages` 셋이 같은 push 에서 서로 독립적으로 떠서 **테스트가 붉어도 배포는 라이브까지 나갔다.** `deploy-pages.yml` 을 `push: main` 에서 `workflow_run`(CI 완료) 으로 바꿨고, `workflow_dispatch` 는 owner 수동 override 로 남겼다.
  - **적대적 리뷰가 값을 했다 — 내 수정이 blocker 를 하나 새로 만들었다.** 세 각도가 독립적으로 같은 결론에 도달했고 전제를 실측으로 확인했다: 이 저장소는 `private=false`·`allow_forking=true` 이고 `ci.yml` 은 `pull_request` 에서도 돈다. `workflow_run` 의 `branches: ["main"]` 필터는 트리거한 run 의 **head_branch** 를 보므로 **포크의 브랜치 이름이 `main` 이면 통과한다.** 첫 배선의 `if:` 는 `conclusion` 만 봤으므로, 포크 커밋이 `head_sha` 로 체크아웃돼 `npm ci` + `build:pages` 를 거쳐 **머지 없이 라이브로 나갈 수 있었다.** 기존 `push: main` 에는 없던 위험이다(push 는 write 권한이 전제). `event == 'push'` + `head_repository.full_name == github.repository` + `head_branch == 'main'` 세 조건을 모두 요구하도록 좁혔다.
  - **`cancel-in-progress: true` → `false`.** GitHub 공식 Pages 스타터가 이 자리에 false 를 쓴다. concurrency 는 **run 단위** 이고 job `if` 는 그 뒤에 평가되므로, CI 가 붉어 skip 될 run 도 `pages` 그룹에 먼저 들어가 **진행 중이던 정상 배포를 취소한 뒤 자기는 skip 된다** — owner 화면에는 회색 2건, 알림 0건. 변경 전에도 있던 설정이지만 push 배선에서는 무해했다(push 순서 = 배포 순서).
  - **shallow clone 을 런타임에 붉힌다.** 체크아웃이 `push 이벤트 + 브랜치 ref` 에서 `workflow_run + bare SHA` 로 바뀌면서 full clone 정당화의 전제가 달라졌다. shallow 로 떨어지면 sitemap lastmod 147건이 조용히 배포 시각이 된다(`scripts/git-last-modified.ts`). 유닛 테스트는 fallback **동작** 만 잠그므로, 실제 러너의 클론 상태를 `git rev-parse --is-shallow-repository` 로 확인해 아니면 `exit 1` 한다.
  - **계약 테스트 신설 — `scripts/deploy-gate-contract.test.ts` 9건.** `workflow_run` 배선은 **조용히** 깨진다: 참조 워크플로 이름이 바뀌면 트리거가 아무 소리 없이 멈추고(배포 실패가 아니라 배포 **부재**), `ref` 고정이 빠지면 CI 가 통과한 커밋과 배포 커밋이 갈라지며, `ci.yml` 의 `push: main` 세 줄이 사라지면 배포가 영구히 멈추는데 다른 테스트는 전부 green 이다. 문법은 유효하므로 GitHub 은 경고하지 않는다 — 그래서 테스트가 잡는다. **회귀 6건을 실제로 주입해 각각 의도한 테스트 하나씩만 붉어지는 것을 확인했다**(push 트리거 복귀 · CI 이름 불일치 · `ref` 고정 제거 · 포크 가드 제거 · `cancel-in-progress: true` · `ci.yml` push 제거).
  - **이 변경이 만든 새 침묵 — 후속 필요.** 게이트를 붙이면서 **skip 이라는 조용한 실패 모드**가 생겼다. CI 가 `failure`·`cancelled`·`action_required` 로 끝나면 배포 run 은 회색 skip 으로 끝나고 GitHub 은 skip 에 알림을 보내지 않는다. 이전 `push` 배선에서는 커밋마다 Deploy 행이 반드시 하나 생겨 부재가 눈에 띄었다. 리뷰 제안은 `dist/build-info.json`(배포 커밋 스탬프) + 일일 freshness 워크플로로 "라이브가 main 보다 뒤처졌다"를 붉히는 것이다. 새 워크플로 신설이라 Phase 2.5 `새 파이프라인 도입` 가드레일과의 경계 판단이 필요해 이 PR 에 넣지 않았다 — **owner 판단 항목으로 올린다.**
  - **남은 low 항목**: CI 는 GA 없이 빌드하고 배포는 `VITE_GA_MEASUREMENT_ID` 를 주입한다(프로덕션 번들 자체는 CI 가 증명하지 않음). `Verify Release` 는 이제 아무것도 게이트하지 않는데 그 사실이 문서화돼 있지 않다. `workflow_dispatch` 복구 절차도 미문서화다.
  - 게이트: `health:all` green(300 unit + e2e 28, content 44, seo 23), `check:consistency` 0/0, `audit:facts` 6/6.
...
- `JapTm` operating-copy maintenance
  - registry 정본 baseline: `15 chapters / 185 search entries / growth / mature / full QA` (2026-06-30 #114로 `incubate→growth`·`beta→mature`·`standard→full` 승급, factsReviewedOn 2026-08-15 · verifiedOn re-stamp 2026-08-25). 이전 `145 / incubate / beta / standard` 표기는 승급 이전 잔재였다.
...
- `UKTm` operating-copy maintenance
  - registry 정본 baseline: `15 chapters / 183 search entries / growth / mature / full QA` (2026-07-07 #121로 `incubate→growth`·`beta→mature`·`standard→full` 승급, factsReviewedOn 2026-08-30 · verifiedOn 2026-08-25). 이전 `14 / 128 / incubate / beta / standard` 표기는 승급 이전 잔재였다(2026-06-03 명칭 drift 기록은 아래 `Resolved Drift Archive`에 보존).
...
- `UsaTm` operating-copy maintenance
  - registry 정본 baseline: `15 chapters / 206 search entries / growth / mature / full QA` (factsReviewedOn 2026-08-15 · verifiedOn re-stamp 2026-08-25). mature closeout: 집행 포럼 통합 플레이북(15장) 신설 + claim-map 15건 + full-QA 파이프라인 편입 + stale/pending 사실과 Jack Daniel's/CBP 후속 claim 1차출처 재대조 완료.
