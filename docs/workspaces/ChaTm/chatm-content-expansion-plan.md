...
- 현재 병목은 "초기 장 수 확보"가 아니라, mature lane scorecard evidence와 문서 정합성을 계속 유지하는 일이다.
...
- 검색 엔트리: 358 current baseline, growth lane mature baseline 유지 기준 확보
...
- 월간 scorecard review 반영 뒤 `ChaTm`은 registry 기준 `mature`로 승격됐고, `health:report` 기준 verdict도 `hold`로 정렬됐다.
