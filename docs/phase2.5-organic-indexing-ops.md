...
|------|------|-------------------------|
| sitemap | live, **143 URL** | `curl -s https://ywkinfo.github.io/glotm/sitemap.xml \| grep -c '<loc>'` → 143 |
...
| briefs (index 1 + 최신 issue 14) | 15 | ★ 최신호(`2026-07-*` 등) 위주 |
...
2. **Sitemaps**에서 `https://ywkinfo.github.io/glotm/sitemap.xml` 제출/갱신 → status `Success`·discovered 143 확인.
...
- **Sitemap server health와 SC 처리 상태를 분리한다.** `https://ywkinfo.github.io/glotm/sitemap.xml`가
  `HTTP 200`, `content-type: application/xml`, 143 `<loc>`를 반환하면 배포 산출물은 정상이다. SC의
...
- [ ] §0 readiness 재확인(sitemap 143 / robots / GA id)
