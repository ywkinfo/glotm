...
|------|------|-------------------------|
| sitemap | live, **149 URL** (2026-08-31 로컬 release 산출 기준) | `curl -s https://ywkinfo.github.io/glotm/sitemap.xml \| grep -c '<loc>'` → 149 |
...
### sitemap URL 인벤토리 (149 = SC 색인 대상, 2026-08-31 기준)
...
2. **Sitemaps**에서 `https://ywkinfo.github.io/glotm/sitemap.xml` 제출/갱신 → status `Success` 확인. discovered 수는 위 인벤토리 표(현재 149)와 대조한다.
...
- **Sitemap server health와 SC 처리 상태를 분리한다.** `https://ywkinfo.github.io/glotm/sitemap.xml`가
  `HTTP 200`, `content-type: application/xml`, 인벤토리 표와 같은 수의 `<loc>`(현재 149)를 반환하면 배포 산출물은 정상이다. SC의
...
- [ ] §0 readiness 재확인(sitemap 149 / robots / GA id)
