...
|------|------|-------------------------|
| sitemap | live, **151 URL** (2026-09-11 로컬 release 산출 기준) | `curl -s https://ywkinfo.github.io/glotm/sitemap.xml \| grep -c '<loc>'` → 151 |
...
### sitemap URL 인벤토리 (151 = SC 색인 대상, 2026-09-11 기준)
...
| briefs (index 1 + issue 21) | 22 | ★ 최신 3개호 위주 |
...
2. **Sitemaps**에서 `https://ywkinfo.github.io/glotm/sitemap.xml` 제출/갱신 → status `Success` 확인. discovered 수는 위 인벤토리 표(현재 151)와 대조한다.
...
- **Sitemap server health와 SC 처리 상태를 분리한다.** `https://ywkinfo.github.io/glotm/sitemap.xml`가
  `HTTP 200`, `content-type: application/xml`, 인벤토리 표와 같은 수의 `<loc>`(현재 151)를 반환하면 배포 산출물은 정상이다. SC의
...
- [ ] §0 readiness 재확인(sitemap 151 / robots / GA id)
