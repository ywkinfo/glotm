...
| `MexTm` | growth | beta | buyer entry 가치와 실무 밀도 강화, 계획 문서와 우선 장 고정 |
| `EuTm` | validate | beta | fact verification와 문서 정합성 안정화 |
| `UsaTm` | incubate | beta | verification refresh 완료, standard QA 유지 |
| `JapTm` | incubate | pilot | verification refresh 완료, search density 보강 |
...
- `health:runtime`: typecheck, unit tests, runtime smoke
- `health:content`: generated content refresh + ChaTm/MexTm/EuTm local full pipeline 재현
