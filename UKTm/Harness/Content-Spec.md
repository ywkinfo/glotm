...
- 홈/continue reading/search에 연결되는 reader-facing 요약은 현재 `growth` / `mature` / `full QA` 상태와 충돌하지 않게 유지한다(`beta`·`standard QA`·`early-track` 같은 승급 이전 라벨을 현재형으로 쓰지 않는다).
...
- 장 본문이 지나치게 요약형으로 끝나지 않도록, 최소 1개 이상의 구체 예시·운영 시나리오·실행 메모를 포함한다.
- 핵심 기간, 기관명, 제도명, 수수료를 단정적으로 적을 때는 `content/research/uk_tm_fact_verification_log.md`에서 `확정` 상태인지 먼저 확인한다.
- 공식 시스템명은 축약하지 말고 첫 등장 시 정식 명칭을 병기한다. 예: `Search for a trade mark`, `Search UK trade mark classes`, `Trade Marks Journal`, `Madrid Monitor`.
