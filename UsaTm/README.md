...
- 현재 `UsaTm`의 운영 메타데이터는 루트 기준 `incubate / beta / standard QA`이며, 이번 유지보수의 우선순위는 대형 확장보다 lighter-track reader utility와 operating copy 정합화입니다.
- reader utility 정비는 filing basis, specimen, monitoring 흐름을 더 빨리 찾고 이어 읽게 만드는 방향을 우선합니다.
