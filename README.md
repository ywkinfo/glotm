...
- `npm run health:release`: `build + build:pages:glotm`를 묶어 로컬에서 GitHub Pages subpath 출하 전 상태를 검증한다. 이 lane에서는 prerendered HTML, `dist/sitemap.xml`, `dist/robots.txt`, `dist/404.html`, `dist/.nojekyll`와 favicon asset 출하 상태까지 함께 확인한다. 이 검증은 local release artifact 기준이며, live Pages 상의 개별 상호작용 이슈까지 자동으로 보증하는 표현으로 읽지 않는다.
