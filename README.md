...
- `npm run health:release`: `build + build:pages:glotm`를 묶어 GitHub Pages subpath 출하 전 상태를 검증한다. 이 lane에서는 prerendered HTML, `dist/sitemap.xml`, `dist/robots.txt`, `dist/404.html`, `dist/.nojekyll`까지 함께 확인한다.
