...
- `npm run health:content`: 루트 `content:prepare` 뒤에 `test:content`와 `ChaTm`·`MexTm`·`EuTm`의 workspace local full pipeline을 다시 재현한다. `UsaTm`·`JapTm`은 lighter-track(shortcut) 기준으로 루트 `content:prepare`의 refresh 계약을 따른다. `UKTm`은 루트에서 full pipeline을 사용하지만 `health:content`에서 workspace-local 추가 실행은 포함하지 않는다.
