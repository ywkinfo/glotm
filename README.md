...
루트 verification 계약은 3개 execution lane(`health:runtime`, `health:content`, `health:release`)으로 운영하고, `health:report`는 최근 lane 상태와 provenance를 묶는 reporting surface로 유지합니다. `npm run content:prepare`는 그 앞단에서 generated content를 맞추는 prerequisite입니다.
...
- `npm run health:content`: 루트 `content:prepare` 뒤에 `test:content`와 `ChaTm`·`MexTm`·`EuTm`·`UsaTm`의 workspace local full pipeline을 다시 재현한다. `JapTm`은 lighter-track(shortcut) 기준으로 루트 `content:prepare`의 refresh 계약을 따른다. `UKTm`은 루트에서 full pipeline을 사용하지만 `health:content`에서 workspace-local 추가 실행은 포함하지 않는다.
...
- `npm run content:prepare`: 7개 가이드의 generated content와 Reports generated content를 재생성한 뒤 `scripts/sync-generated-content.mjs`로 루트 셸 소비 경로를 동기화한다. 현재 루트 기준으로 `LatTm`·`MexTm`·`UsaTm`·`ChaTm`·`EuTm`·`UKTm`은 `build-master -> qa-content -> build-content` 전체 흐름을 타고, `JapTm`은 shortcut generated-content 갱신 경로를 사용하며, Reports는 `Reports/scripts/build-content.ts`를 실행한다. `JapTm`의 더 깊은 콘텐츠 검토가 필요하면 워크스페이스 로컬 `content:prepare`를 직접 실행한다.
...
루트 워크스페이스 명령은 각 가이드의 generated JSON을 현재 검증 계약에 맞게 재생성합니다. 다만 실행 깊이는 가이드마다 다릅니다. 현재 `LatTm`·`MexTm`·`UsaTm`·`ChaTm`·`EuTm`·`UKTm`은 루트에서도 full pipeline을 사용하고, `JapTm`만 루트에서 shortcut 예외 그룹으로 동작합니다. 별도로 `npm run content:reports`는 Report / Gateway trust layer가 읽는 reports generated JSON을 갱신합니다. `JapTm`의 콘텐츠를 직접 수정하거나 deeper content QA가 필요할 때는 로컬 `content:prepare`를 우선 실행하고, 편집 작업 전에는 각 가이드의 `README.md`와 `package.json`을 함께 확인합니다.
...
현재 포트폴리오의 기본 실행 순서는 `ChaTm -> MexTm -> EuTm -> Report / Gateway trust layer -> UsaTm -> JapTm -> UKTm`입니다. `LatTm`은 기준 프레임 보호를 우선하며, 대형 신규 집필보다 freshness·density·reader QA 유지에 집중합니다.
