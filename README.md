...
- `npm run health:content`: 루트 `content:prepare` 뒤에 `test:content`와 `ChaTm`·`MexTm`·`EuTm`·`UsaTm`·`JapTm`의 workspace local full pipeline을 다시 재현한다. `UKTm`은 루트에서 full pipeline을 사용하지만 `health:content`에서 workspace-local 추가 실행은 포함하지 않는다.
...
루트 워크스페이스 명령은 각 가이드의 generated JSON을 현재 검증 계약에 맞게 재생성합니다. 현재 `LatTm`·`MexTm`·`UsaTm`·`JapTm`·`ChaTm`·`EuTm`·`UKTm` 전부 루트에서도 full pipeline을 사용합니다. 별도로 `npm run content:reports`는 Report / Gateway trust layer가 읽는 reports generated JSON을 갱신합니다. 각 가이드의 콘텐츠를 직접 수정하거나 deeper content QA가 필요할 때는 로컬 `content:prepare`를 우선 실행하고, 편집 작업 전에는 각 가이드의 `README.md`와 `package.json`을 함께 확인합니다.
