...
- 현재 lane 목표는 `mature`와 `full QA`를 growth lane으로 안정적으로 유지하는 것이다(2026-06-29 승급). 대형 신규 집필보다 claim-map·1차출처 freshness와 filing/examination·consent/maintenance/enforcement reader utility를 루트 리더와 어긋나지 않게 유지하는 쪽을 우선한다.
...
- workspace 문서와 리더 copy는 `mature` growth 상태를 전제로 유지하고, draft 공개본처럼 읽히는 표현이나 대형 확장 전제를 불필요하게 넣지 않는다.
